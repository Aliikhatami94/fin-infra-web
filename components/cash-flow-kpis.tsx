...
import { useCallback, useMemo, useRef, useState, type KeyboardEvent, type MouseEvent, useEffect } from "react"
...
export function CashFlowKPIs() {
  ...
  const kpiCards = [
    ...
    },
    {
      ...
      sparklineColor: "var(--color-positive)",
      showChange: false,
    },
    {
      label: "Total Outflow",
      ...
      <div className="space-y-1">
        ...
        <AnimatePresence>
          {!isCollapsed && (
            ...
            >
              {/* Time scale selector */}
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border/60 bg-card/30 px-4 py-3 mb-4">
                <div>
                            ...
                            <Card className="card-standard card-lift h-full min-h-[260px] md:min-h-[280px]">
                              <CardContent className="flex flex-col h-full gap-3 p-4 md:p-6">
                                ...
                                <div className="flex items-start justify-between gap-3">
                                  ...
                                  <KPIIcon icon={kpi.icon} tone={kpi.tone} size="md" />
                                </div>
                                {!isHidden && (
                                    ...
                                    <div className="w-20 h-10">
                                      <ResponsiveContainer width="100%" height="100%" aria-hidden="true">
                                        <LineChart data={kpi.sparkline}>
                                          <Line type="monotone" dataKey="value" stroke={kpi.sparklineColor} strokeWidth={2} dot={false} />
                                        </LineChart>
                                      </ResponsiveContainer>
                                    </div>
                                  </div>
              ...
              <div className="hidden md:grid grid-cols-2 xl:grid-cols-3 gap-4 auto-rows-fr">
                {kpiCards.map((kpi, index) => {
                  const isHidden = hiddenCards.has(kpi.label)
                  return (
                    <motion.div key={kpi.label} {...createStaggeredCardVariants(index, 0)} {...cardHoverVariants} className="h-full">
                      <Card className="card-standard card-lift h-full min-h-[260px] md:min-h-[280px]">
                        <CardContent className="flex flex-col h-full gap-3 p-4 md:p-6">
                          <div className="flex items-start justify-between gap-2">
                            <LastSyncBadge timestamp="2 min ago" source="Plaid" />
