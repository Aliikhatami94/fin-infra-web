...
export function LastSyncBadge({ timestamp, source, className }: LastSyncBadgeProps) {
  ...
  return (
    <div
      className={cn(
        "inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] text-muted-foreground max-w-full",
        isOffline ? "bg-amber-500/15 text-amber-900 dark:text-amber-100" : "bg-muted/50",
        className,
      )}
    >
      {isOffline ? <WifiOff className="h-2.5 w-2.5 shrink-0" /> : <Clock className="h-2.5 w-2.5 shrink-0" />}
